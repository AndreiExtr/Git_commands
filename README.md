...
$${\color{red}Создается папка dist при сборке Vue.js проекта. Эта папка содержит скомпилированные файлы приложения, которые будут загружены на GitHub Pages.}$$
$${\color{red}Прежде чем запушить проект на удаленный репозиторий, надо в файле <code>.gitignore</code> удалить <code>/dist</code>.}$$
